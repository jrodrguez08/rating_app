...
- Derive rating eligibility only from confirmed match participation: starters and substitutes with provider evidence of entering are rateable; unused or unconfirmed substitutes never are. Keep participant DTOs behind `FootballDataProvider`, preserve participant/coach sync idempotency, and allow late provider data to correct participation before lifecycle code consumes it.
