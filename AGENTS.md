...
- Use Firebase UID as the canonical `voterId`; reuse the persistent anonymous identity and do not expose it or add visible login/reset friction. Authentication grants no general Firestore access, and future ballots must bind their deterministic `voterId` path to `request.auth.uid`.
