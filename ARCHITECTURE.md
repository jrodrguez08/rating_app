...
For a specific persisted match, the participant sync makes one `GET /fixtures?id=...` request. The embedded tracked-Team lineup establishes starters, bench membership, and head coach; stable-ID substitution events confirm entering substitutes and entry/exit minutes; positive player-stat minutes provide corroborating participation when that block is populated. Starters participate by definition. A bench player without either substitution or positive-minute evidence remains `participated: false`; missing data is never guessed. Duplicate events reconcile by provider player ID. The full response validates before writes, and deterministic provider-mapped IDs make Player, participant, Coach, and the fixed head-coach assignment upserts idempotent while preserving `createdAt`. Re-running after provider finalization may update snapshots and change a previously unconfirmed substitute to participating before voting lifecycle code consumes the records.
