...
After fixture sync, choose a persisted internal match ID and run `npm run sync:match-participants -- <match-id>`. The command loads that Match and its tracked Team, makes one combined fixture-detail request, and upserts `players`, match-scoped `participants`, `coaches`, and the deterministic `head-coach` assignment. Starters are confirmed participants. Bench players become participants only through a stable-ID substitution event or positive provider minutes; unused or unconfirmed substitutes remain non-rateable. Repeated runs preserve identities and `createdAt`, while later provider data may safely improve participation and minute snapshots. Missing lineups, tracked-Team data, or coach data fail without inventing eligibility.
